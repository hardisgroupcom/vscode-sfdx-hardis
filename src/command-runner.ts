...
export class CommandRunner {
     ...
     */
    private activeCommands: Map<string, { type: 'background' | 'terminal', process?: any, sentToTerminal?: boolean }> = new Map();
    ...
    preprocessAndValidateCommand(command: string, type: 'background' | 'terminal' = 'background', process?: any): string | null {
        ...
        if (
            ...
            if (this.commandsInstance.disposableWebSocketServer &&
                this.commandsInstance.disposableWebSocketServer.websocketHostPort !== null) {
        ...
        if (existing) {
            ...
            if (existing.type === 'background' && existing.process && !existing.process.killed && !existing.process._closed) {
                vscode.window.showErrorMessage("No need to click multiple times on a menu, just be patient 🤗");
                return null;
            }
    ...
    executeCommandBackground(sfdxHardisCommand: string) {
        ...
        preprocessedCommand = this.preprocessAndValidateCommand(sfdxHardisCommand, 'background');
        ...
        this.activeCommands.set(preprocessedCommand, { type: 'background', process: childProcess });
        // Create or show a VS Code output channel for background command output
        if (!this.outputChannel) {
            this.outputChannel = vscode.window.createOutputChannel("SFDX Hardis Commands");
        }
                ...
                if (!killed) {
                    ...
                    if (childProcess && childProcess.pid) {
                        try {
                            ...
                            output.appendLine("[Cancelled by user] Error killing child process:" + error.message);
                            progressResolve && progressResolve();
                        }
                    } else {
                        progressResolve && progressResolve();
    ...
    runCommandInTerminal(command: string) {
        ...
        if (!terminal) {
            vscode.window.showErrorMessage("No terminal available to run the command.");
            return;
        }
        terminal.show(false);
