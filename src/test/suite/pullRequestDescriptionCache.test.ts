...
suite("Pull Request description cache", () => {
  ...
  suite("repositoryKeyFromRemoteUrl()", () => {
    ...
    test("drops the protocol, the credentials, the .git suffix and the case", () => {
      ...
      for (const remote of [
        ...
        assert.strictEqual(repositoryKeyFromRemoteUrl(remote), expected, remote);
      }
    });
  ...
  suite("normalizeTerminalState()", () => {
    test("recognises the merged state of the four providers", () => {
      for (const state of ["merged", "MERGED", "completed", 3]) {
        assert.strictEqual(normalizeTerminalState(state), "merged", `state ${state}`);
      }
    });

    test("recognises the closed state of the four providers", () => {
      for (const state of ["closed", "DECLINED", "abandoned", "SUPERSEDED"]) {
        assert.strictEqual(normalizeTerminalState(state), "closed", `state ${state}`);
      }
    });
    ...
    test("refuses every non terminal state", () => {
      for (const state of ["open", "active", "OPEN", "", null, undefined, "unknown"]) {
        assert.strictEqual(normalizeTerminalState(state), null, `state ${state}`);
      }
    });
  });

  test("stores nothing for an open Pull Request, and reads nothing back", () => {
    setCachedPullRequestDescription("azure", REPO, 42, "active", "still being written");
    assert.strictEqual(fs.existsSync(cacheFile()), false);
    assert.strictEqual(getCachedPullRequestDescription("azure", REPO, 42, "active"), null);
  });

  test("round-trips the description of a merged Pull Request", () => {
    const description =
      "Promotion branch\n\n```yaml\npromotionPullRequests: [12, 34]\n```";
    setCachedPullRequestDescription("azure", REPO, 38, "completed", description);
  ...
  test("ignores a cached entry when the Pull Request is open again", () => {
    setCachedPullRequestDescription("azure", REPO, 15, "abandoned", "the declined body");
    assert.strictEqual(getCachedPullRequestDescription("azure", REPO, 15, "active"), null);
  });

  test("ignores a cached entry whose state no longer matches", () => {
    setCachedPullRequestDescription("azure", REPO, 15, "abandoned", "the declined body");
    assert.strictEqual(getCachedPullRequestDescription("azure", REPO, 15, "completed"), null);
  });
  ...
  test("ignores an entry older than the maximum age", () => {
    ...
    resetPullRequestDescriptionCacheMemory();
    assert.strictEqual(getCachedPullRequestDescription("azure", REPO, 7, "completed"), null);
  });

  test("keeps the entries another process wrote while this one held the file", () => {
    setCachedPullRequestDescription("azure", REPO, 1, "completed", "from this process");
    ...
    setCachedPullRequestDescription("azure", REPO, 3, "completed", "from this process again");

    const merged = JSON.parse(fs.readFileSync(cacheFile(), "utf-8"));
    assert.deepStrictEqual(Object.keys(merged.pullRequests).sort(), ["1", "2", "3"]);
  ...
  test("leaves a file written by a newer version alone", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        repository: REPO,
        pullRequests: {
          "9": { description: "from the future", state: "merged", cachedAt: new Date().toISOString() },
        },
      }),
    );

    resetPullRequestDescriptionCacheMemory();
    assert.strictEqual(getCachedPullRequestDescription("azure", REPO, 9, "completed"), null);
  });
  ...
  test("is bypassed by NO_CACHE and by SFDX_HARDIS_NO_PR_CACHE", () => {
    for (const variable of ["NO_CACHE", "SFDX_HARDIS_NO_PR_CACHE"]) {
      ...
      assert.strictEqual(fs.existsSync(cacheFile()), false, `${variable} still wrote the file`);
      assert.strictEqual(getCachedPullRequestDescription("azure", REPO, 5, "completed"), null);
  ...
  test("sanitises a key into a safe file name", () => {
    assert.strictEqual(
