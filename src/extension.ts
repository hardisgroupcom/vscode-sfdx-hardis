...
export function activate(context: vscode.ExtensionContext) {
  ...
  vscode.commands.registerCommand("vscode-sfdx-hardis.openKeyFile", async () => {
    const keyFileList = [
      { file: 'config/.sfdx-hardis.yml', label: 'sfdx-hardis main configuration file' },
      { file: 'manifest/package.xml', label: 'List of all deployed metadatas' },
      { file: 'manifest/packageDeployOnce.xml', label: 'List of metadatas that will be deployed only if not existing in the target org' },
      { file: 'config/project-scratch-def.json', label: 'Scratch org creation definition file' },
    ...
    const value = await new Promise<any>((resolve) => {
      ...
      quickpick.items = keyFileList
        .filter((choice: any) => fs.existsSync(currentWorkspaceFolderUri + path.sep + choice.file))
        .map((choice: any) => {
    ...
    quickpick.dispose();
    if (value) {
      var openPath = vscode.Uri.parse("file:///" + currentWorkspaceFolderUri + '/' + value);
