...
export class Commands {
      ...
      if (dirent.isDirectory()) {
        ...
        if (fs.existsSync(exportJsonPath)) {
          try {
            ...
            Logger.log(`Error reading export.json for workspace ${dirent.name}: ${error}`);
          }
        }
      }
    }
  ...
  private async updateFilesWorkspace(data: any): Promise<void> {
    ...
    vscode.window.showInformationMessage(`Files workspace "${data.label}" updated successfully!`);
  }

  private async deleteFilesWorkspace(workspacePath: string): Promise<void> {
    if (fs.existsSync(workspacePath)) {
      await fs.remove(workspacePath);
      vscode.window.showInformationMessage('Files workspace deleted successfully!');
    }
  }
