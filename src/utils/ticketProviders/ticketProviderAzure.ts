...
export class AzureBoardsProvider extends TicketProvider {
    ...
    async authenticate(): Promise<boolean | null> {
        ...
        if (!gitProvider || !gitProvider.isActive) {
            Logger.log("Git provider is not active. Azure Boards requires an authenticated Azure DevOps git connection.");
            return false;
        }

        if (gitProvider.repoInfo?.providerName !== "azure") {
            Logger.log("Current git repository is not Azure DevOps. Azure Boards provider requires Azure DevOps repository.");
            return false;
        }
        ...
        const urlMatch = azureGitProvider.repoInfo.webUrl?.match(/^(https?:\/\/[^/]+\/[^/]+)/);
        if (!urlMatch) {
            Logger.log("Could not extract Azure DevOps organization URL from repository info.");
            return false;
        }
        ...
        try {
            ...
            Logger.log(`Azure Boards authentication successful (Project: ${this.teamProject})`);
            return true;
        } 
        catch (error: any) {
            Logger.log(`Azure Boards authentication failed: ${error?.message || String(error)}`);
            this.azureApi = null;
    ...
    async completeTicketDetails(ticket: Ticket): Promise<Ticket> {
        if (!this.isAuthenticated || !this.workItemApi) {
            Logger.log("Azure Boards client not authenticated. Call authenticate() first.");
            return ticket;
        }
        ...
        try {
            ...
            const workItem = await this.workItemApi.getWorkItem(workItemId, undefined, undefined, undefined, this.teamProject);
            ...
            Logger.log(`Error fetching Azure Boards Work Item ${ticket.id}: ${error?.message || String(error)}`);
