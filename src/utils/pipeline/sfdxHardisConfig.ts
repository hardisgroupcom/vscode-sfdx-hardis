...
export const setConfig = async (layer: string, propValues: any): Promise<void> => {
  if (layer === 'user' && (fs.readdirSync(process.cwd()).length === 0 || !isGitRepo())) {
    console.warn("log", this, c.grey('Skip update user config file because current directory is not a salesforce project'));
    return;
  }
  const workspaceRoot = getWorkspaceRoot()
  const userConfigFilesWithWorkspaceRoot = userConfigFiles.map((file) => {
...
export async function getCurrentGitBranch(options: any = { formatted: false }) {
  ...
  const gitBranch = process.env.CI_COMMIT_REF_NAME || (await simpleGit().branchLocal()).current;
