/*
Local cache of Pull Request descriptions, shared with the sfdx-hardis CLI.

Why it exists: Azure DevOps truncates the description of a Pull Request returned by its LIST API at
400 characters, so the DevOps Pipeline has to read each long Pull Request again to get the
`promotionPullRequests` declaration of a promotion branch. On a repository with hundreds of Pull
Requests that is hundreds of round trips every time the pipeline is opened or refreshed.
...
THE ON-DISK CONTRACT IS SHARED WITH sfdx-hardis. Its implementation lives in
`src/common/cache/pullRequestDescriptionCache.ts` and must be changed with this one. A cache warmed
by a CLI job is read by the extension, and the other way round:
...
// A repository file holds at most this many entries: beyond it the oldest are dropped
...
// One in-memory copy per repository key, so a pipeline refresh reads the file once
...
export function pullRequestCacheFile(
  provider: string,
  repositoryKey: string,
): string {
  return path.join(
    pullRequestCacheDir(),
    `${sanitizeRepositoryKey(provider)}__${sanitizeRepositoryKey(repositoryKey)}.json`,
  );
}
...
function readCacheFile(
  ...
  const loaded: PullRequestCacheFile = {
 ...
 */
export function getCachedPullRequestDescription(
  provider: string,
  repositoryKey: string,
  ...
  if (isPullRequestDescriptionCacheDisabled() || state === null) {
    return null;
  }
  const cache = readCacheFile(provider, repositoryKey);
  const entry = cache.pullRequests[String(pullRequestId)];
  if (!entry || isExpired(entry) || entry.state !== state) {
    return null;
  }
 ...
 * renamed, so a CLI job and the extension writing at the same time cannot lose each other's
 * entries or leave a half-written file behind.
 */
export function setCachedPullRequestDescription(
  provider: string,
  repositoryKey: string,
  pullRequestId: number | string,
  currentState: string | number | null | undefined,
  ...
  const memoryKey = `${provider}__${repositoryKey}`;
  const cache = readCacheFile(provider, repositoryKey);
  cache.pullRequests[String(pullRequestId)] = {
    description,
    state,
