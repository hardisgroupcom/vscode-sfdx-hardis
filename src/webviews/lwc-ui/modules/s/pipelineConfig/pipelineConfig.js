				...
				.map(section => {
					...
					for (const key of section.keys) {
						...
						if (schema.enum) {
							isEnum = true;
							options = schema.enum;
							enumNames = Array.isArray(schema.enumNames) ? schema.enumNames : null;
							optionsLwc = schema.enum.map((opt, idx) => ({
								label: enumNames && enumNames[idx] ? String(enumNames[idx]) : String(opt),
								value: String(opt)
							}));
						} else if (schema.type === 'array' && schema.items && schema.items.enum) {
							isArrayEnum = true;
							options = schema.items.enum;
							enumNames = Array.isArray(schema.items.enumNames) ? schema.items.enumNames : null;
						...
						if (isArrayEnum) {
							...
							if (enumNames && options && options.length > 0) {
								...
								valueDisplay = idx !== -1 && enumNames[idx] ? enumNames[idx] : value;
							} else {
								valueDisplay = value;
							}
						} else {
