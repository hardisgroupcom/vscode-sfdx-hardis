...
- Org Monitoring: in a monitoring repository, the panel shows the CI/CD repository that deploys to the org (`deploymentRepository`) and opens it in a new VS Code window, or invites you to set it with [hardis:org:configure:monitoring-deployment-repository](https://sfdx-hardis.cloudity.com/hardis/org/configure/monitoring-deployment-repository/) so that [coding agents](https://sfdx-hardis.cloudity.com/salesforce-monitoring-metadata-backup/#ask-questions-with-a-coding-agent) can search it too.
...
- **Backpromote (Beta) panel**: deploy into your developer sandbox what your teammates merged in the parent branch, from the DevOps Pipeline or the commands menu ([guide](https://sfdx-hardis.cloudity.com/salesforce-devops-backpromote/))
...
- **DevOps Pipeline: [promotion branches](https://sfdx-hardis.cloudity.com/salesforce-devops-promotion-branches/) (experimental)** (sfdx-hardis `enablePromotionBranches`, off by default)
...
- **Run Local HTML Doc Pages** now tells you what is wrong instead of showing "Starting local documentation server..." for ever
  - Documentation menus generated by an earlier version cannot be read by Zensical: the affected menus are now named, and a **Regenerate documentation** button repairs them (needs the matching sfdx-hardis version)
  - A missing or unreadable `mkdocs.yml` is reported before the server is started, with a button to generate the documentation
  - A **Preparing the local documentation server...** notification says which check is running, and the configuration is checked first, so the answer comes back without waiting for the Python check
- **Documentation Settings** now offers **Doc: Deploy to Confluence**, next to Cloudflare and Salesforce Org
  - When the server still does not answer, you are asked whether to keep waiting instead of being told it failed: a documentation site of several thousand pages can build for a long time, and Zensical shows no progress while it works
...
- The **Command Runner** switches from **Starting** to **Running** as soon as the command process reaches the extension, instead of several seconds later
  - Current sfdx-hardis versions load extra code before announcing themselves; the panel now reacts to their connection itself, which arrives seconds earlier
  - Combined with the next sfdx-hardis release, a command like **New User Story** shows **Running** about one second after the click instead of five
...
- Features opened from the Welcome page now open instantly and load their content inside the panel, instead of leaving you waiting on the Welcome page
  - Applies to Pipeline, Metadata Retriever, Documentation Workbench, Data Workbench, Files Workbench and Extension Configuration
  - If loading fails, the panel shows an error with a Try Again button
...
- Features
  ...
  - The Org Monitoring home and the Monitoring Config Workbench now use the icons returned by `sf hardis:config:monitoring-defaults` for each monitoring command and notification type, instead of a hardcoded per-key icon map; new commands picked up from the CLI now show the right icon automatically
...
- Improve icons of Orgs Manager & DevOps Pipeline views
...
- Display [**useDeltaDeploymentWithDependencies**](https://sfdx-hardis.cloudity.com/salesforce-devops-config-delta-deployment/#delta-with-dependencies-beta) in pipeline configuration
