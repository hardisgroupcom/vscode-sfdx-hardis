...
- **DevOps Pipeline: [promotion branches](https://sfdx-hardis.cloudity.com/salesforce-ci-cd-promotion-branches/) (experimental)** (sfdx-hardis `enablePromotionBranches`, off by default)
  ...
  - `allowedPromotionSteps` (Danger Zone) declares the source and target branches a release manager can promote between: **Create promotion** then shows up on those source branches only, and a step with a single allowed target passes it to the command instead of asking
