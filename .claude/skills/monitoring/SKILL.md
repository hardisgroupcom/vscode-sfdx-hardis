...
1. **`resources/global-theme.css`** — already loaded by every webview (see `src/webviews/lwc-ui-panel.ts:894`). It ships `.header-section`, `.header-content`, `.header-text`, `.header-title`, `.header-subtitle`, `.header-icon-container.{green,teal,gray,blue,purple,orange,yellow}`, plus the generic section & card kit used by both monitoring pages: `.hardis-group-label`/`.hardis-group-desc`, `.hardis-card-grid`, `.hardis-card` (+ `.clickable`), `.hardis-card-head/-title/-desc/-arrow`, and `.hardis-tile` icon tiles whose hue classes accept the catalog `colorClass` values (`backup`, `audit`, `security`, `tests`, `apex`, `limits`, ...) — all pre-themed via `.slds-scope[data-theme="light"|"dark"]`. Reuse, don't redefine. Never use the deprecated `command-card`/`command-icon-container` classes, and never put `variant="inverse"` on a `lightning-icon` inside a `.hardis-tile` (invisible white icon on the soft tint).
