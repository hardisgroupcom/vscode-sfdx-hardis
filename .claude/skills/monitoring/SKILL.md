...
1. **`resources/global-theme.css`** — already loaded by every webview (see `src/webviews/lwc-ui-panel.ts`). It ships the whole design system: the page header kit (`.hardis-page-head` > `.hardis-tile featured <hue>` + `.hardis-page-head-text` > `.hardis-page-title`/`.hardis-page-subtitle` + `.hardis-page-head-actions`), the panel surface `.hardis-panel-body`, the status card kit (`.hardis-status-grid` > `.hardis-status-card` + state), inline notes `.hardis-note`, the section & card kit used by both monitoring pages (`.hardis-group-label`/`.hardis-group-desc`, `.hardis-card-grid`, `.hardis-card` (+ `.clickable`), `.hardis-card-head/-title/-desc/-arrow`), and `.hardis-tile` icon tiles whose hue classes accept the catalog `colorClass` values (`backup`, `audit`, `security`, `tests`, `apex`, `limits`, ...). All of them switch light/dark on their own through the palette tokens. Reuse, don't redefine. The legacy `header-*` / `command-card` / `command-icon-container` classes were deleted; never put `variant="inverse"` on a `lightning-icon` inside a `.hardis-tile` (invisible white icon on the soft tint).
