...
3. Keep flat JSON structure and camelCase keys. Ordering is **case-sensitive ASCII sort** (JavaScript default `sort()`), not case-insensitive alphabetical — uppercase-first keys sort *before* lowercase ones. Locate the real neighbouring keys in `en.json` first, then insert at the same position in all 9 files. Verify with:
   ```bash
